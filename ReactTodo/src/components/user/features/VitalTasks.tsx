...
function VitalTasks() {
    ...
    const [VitalTasks, SetVitalTasks] = useState<TaskProps[]>([]);
    const [SingleTask, SetSingleTask] = useState<TaskProps | null>(null);

    useEffect(() => {
        const fetchVitaltasks = async () => {
            try {
                const response = await authenticatedRequest(`/tasks?category=1`);
                if (!response.success) {
                    toast.error('Something went wrong');
                    return;
                }
                SetVitalTasks(response.data.tasks || []);
            } catch (error) {
    ...
    const handleTaskView = (id: number) => {
        const task = VitalTasks.find((t) => t.id === id);
        if (task) {
            SetSingleTask(task);
        }
    };
